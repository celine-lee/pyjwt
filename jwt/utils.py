...
def _pad_b64(data: Union[bytes, str]) -> bytes:
    """
    Helper to pad a base64url string/binary so its length is a multiple of 4.
    """
...
def from_base64url_uint(val: Union[bytes, str]) -> int:
    decoded = base64.urlsafe_b64decode(_pad_b64(val))
...
_PEM_RE = re.compile(
    b"----[- ]BEGIN (" + b"|".join(_PEMS) + b""")[- ]----\r?.+?\r?----[- ]END \\1[- ]----\r?\n?""",
    re.DOTALL,
)
