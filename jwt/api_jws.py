...
from .utils import ensure_bytes, b64encode, b64decode
...
class PyJWS:
    ...
    def __init__(
        ...
        # Remove algorithms that are not on the whitelist.
    ...
    def register_algorithm(self, alg_id: str, alg_obj: Algorithm) -> None:
        """
        Registers a new Algorithm for use in signing and verifying tokens.
        """
    ...
    def get_algorithms(self) -> list[str]:
        """
        Returns a list of supported 'alg' values.
        """
        return list(self._valid_algs)

    def get_algorithm_by_name(self, alg_name: str) -> Algorithm:
        """
        Returns the Algorithm object corresponding to the given name.
        """
        try:
    ...
    def encode(
        ...
        segments.append(b64encode(json_header))

        if is_payload_detached:
            msg_payload = payload
        else:
            msg_payload = b64encode(payload)
        ...
        if header.get("b64", True) is False:
            if detached_payload is None:
                raise DecodeError(
                    ...
                    "to decode a message having the b64 header set to false."
                )
    ...
    def _load(self, jwt: str | bytes) -> tuple[bytes, bytes, dict[str, Any], bytes]:
        jwt_bytes = jwt if isinstance(jwt, bytes) else jwt.encode("utf-8")

        if not isinstance(jwt_bytes, bytes):
        ...
        try:
            payload = b64decode(payload_segment)
        except (TypeError, binascii.Error) as err:
