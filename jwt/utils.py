...
# -------------------------
# Base64-URL Helper Functions
# -------------------------
def _add_padding(data: bytes) -> bytes:
    """Add missing '=' padding to a base64url-encoded byte string."""
    rem = len(data) % 4
    if rem:
        data += b"=" * (4 - rem)
    return data
...
def base64url_decode(val: Union[bytes, str]) -> bytes:
    ...
    return base64.urlsafe_b64decode(_add_padding(val))
