...
def _encode_segment(data: bytes) -> bytes:
    """Encodes a segment using URL‐safe base64 without padding."""
    return base64url_encode(data)


def _b64_decode_segment(segment: bytes | str) -> bytes:
    """Decodes a base64url segment, applying proper padding if needed."""
    try:
...
class PyJWS:
    ...
    def __init__(
        ...
        self.options = {**self._get_default_options(), **(options or {})}
    ...
    def unregister_algorithm(self, alg_id: str) -> None:
        if alg_id not in self._algorithms:
            raise KeyError("The specified algorithm is not registered.")
    ...
    def encode(
        ...
        if headers:
            ...
            if "b64" in headers and headers["b64"] is False:
        ...
        segments.append(_encode_segment(json_header))

        if is_payload_detached:
            msg_payload = payload
        else:
            msg_payload = _encode_segment(payload)
        ...
        segments.append(_encode_segment(signature))
    ...
    def decode_complete(
        ...
        **kwargs,
        ...
        if verify_signature and not algorithms:
            raise DecodeError('You must pass a value for the "algorithms" argument when verifying a token.')

        payload, signing_input, header, signature = self._load(jwt)

        if header.get("b64", True) is False:
            if detached_payload is None:
                raise DecodeError('A "detached_payload" argument is required if the header parameter "b64" is false.')
    ...
    def _load(self, jwt: str | bytes) -> tuple[bytes, bytes, dict[str, Any], bytes]:
        ...
        if not isinstance(jwt, bytes):
            raise DecodeError("Invalid token type. Token must be bytes.")
        ...
        try:
            header_bytes = _b64_decode_segment(header_segment)
        except (TypeError, binascii.Error) as err:
        ...
        try:
            payload_bytes = _b64_decode_segment(payload_segment)
        except (TypeError, binascii.Error) as err:
            raise DecodeError("Invalid payload padding") from err

        try:
            crypto_bytes = _b64_decode_segment(crypto_segment)
        except (TypeError, binascii.Error) as err:
