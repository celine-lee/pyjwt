...
def _jwk_to_bytes(val: Union[str, bytes]) -> bytes:
    return b64decode(val)


def _bytes_to_jwk(val: bytes) -> str:
...
class HMACAlgorithm(Algorithm):
    """
    Signing and verification operations using HMAC.
    """
    ...
    def to_jwk(key_obj: str | bytes, as_dict: bool = False) -> Union[JWKDict, str]:
        key_as_bytes = key_obj.encode("utf-8") if isinstance(key_obj, str) else key_obj
        jwk = {
            "k": b64encode(key_as_bytes).decode("utf-8"),
            "kty": "oct",
        }
...
if has_crypto:
    ...
    class RSAAlgorithm(Algorithm):
        """
        Signing and verification using RSASSA-PKCS-v1_5.
        """
    ...
    class ECAlgorithm(Algorithm):
        ...
        def from_jwk(jwk: str | JWKDict) -> AllowedECKeys:
            ...
            if len(d) != len(x):
                raise InvalidKeyError(
                    "D should be {} bytes for curve {}".format(len(x), curve)
                )
    ...
    class OKPAlgorithm(Algorithm):
        """
        Signing and verification using EdDSA.
        """
