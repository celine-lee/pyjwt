...
# Helper functions for byte conversions and base64url encoding/decoding
def ensure_bytes(val: Union[bytes, str]) -> bytes:
...
def b64url_encode_nopad(data: bytes) -> bytes:
    return base64.urlsafe_b64encode(data).replace(b"=", b"")


def b64url_decode_nopad(val: Union[bytes, str]) -> bytes:
    data = ensure_bytes(val)
    rem = len(data) % 4
    if rem:
        data += b"=" * (4 - rem)
...
def to_base64url_uint(val: int) -> bytes:
    ...
    if not int_bytes:
        int_bytes = b"\x00"
    return b64url_encode_nopad(int_bytes)


def from_base64url_uint(val: Union[bytes, str]) -> int:
    data = b64url_decode_nopad(val)
...
# PEM and SSH key regexes and checks
_PEMS = {
    b"CERTIFICATE",
