...
class HMACAlgorithm(Algorithm):
    ...
    def to_jwk(key_obj: str | bytes, as_dict: bool = False) -> Union[JWKDict, str]:
        ...
        jwk = {
            "k": b64url_encode_nopad(key_as_bytes).decode(),
            "kty": "oct",
        }
    ...
    def from_jwk(jwk: str | JWKDict) -> bytes:
        ...
        return b64url_decode_nopad(k_as_bytes)
    ...
    class ECAlgorithm(Algorithm):
        ...
        def from_jwk(jwk: str | JWKDict) -> AllowedECKeys:
            ...
            if "x" not in obj or "y" not in obj:
                raise InvalidKeyError("Not an Elliptic curve key")
            x = b64url_decode_nopad(obj["x"])
            y = b64url_decode_nopad(obj["y"])
            curve = obj.get("crv")
            if curve == "P-256":
                if len(x) == len(y) == 32:
                    curve_obj = SECP256R1()
    ...
    class OKPAlgorithm(Algorithm):
        ...
        def prepare_key(self, key: AllowedOKPKeys | str | bytes) -> AllowedOKPKeys:
            if isinstance(key, (bytes, str)):
                key_str = ensure_bytes(key).decode("utf-8")
                key_bytes = ensure_bytes(key)
            ...
            if not isinstance(
                ...
                raise InvalidKeyError(
                    "Expecting an EdDSA key (Ed25519/Ed448). Wrong key provided."
                )
            return key
        ...
        def to_jwk(key: AllowedOKPKeys, as_dict: bool = False) -> Union[JWKDict, str]:
            ...
            if isinstance(key, (Ed25519PublicKey, Ed448PublicKey)):
                ...
                obj = {
                    "x": b64url_encode_nopad(x).decode(),
                    "kty": "OKP",
                    "crv": crv,
                }
                return obj if as_dict else json.dumps(obj)
            if isinstance(key, (Ed25519PrivateKey, Ed448PrivateKey)):
                ...
                obj = {
                    "x": b64url_encode_nopad(x).decode(),
                    "d": b64url_encode_nopad(d).decode(),
                    "kty": "OKP",
                    "crv": crv,
                }
                return obj if as_dict else json.dumps(obj)
