...
def _to_bytes(data: Union[str, bytes]) -> bytes:
    """
    A helper that converts str to bytes; passes though if already bytes.
    """
...
if has_crypto:
    ...
    class ECAlgorithm(Algorithm):
        ...
        def from_jwk(jwk: str | JWKDict) -> AllowedECKeys:
            ...
            x_as_bytes = _to_bytes(obj.get("x"))
            rem = len(x_as_bytes) % 4
            if rem > 0:
                x_as_bytes += b"=" * (4 - rem)
            x = base64.urlsafe_b64decode(x_as_bytes)

            y_as_bytes = _to_bytes(obj.get("y"))
            ...
            d_as_bytes = _to_bytes(obj.get("d"))
    ...
    class OKPAlgorithm(Algorithm):
        ...
        def prepare_key(self, key: AllowedOKPKeys | str | bytes) -> AllowedOKPKeys:
            ...
            if not isinstance(
                ...
                raise InvalidKeyError(
                    "Expecting a valid key for EdDSA algorithms."
                )
            return key
