...
def _b64_decode_segment(segment: bytes | str) -> bytes:
    """
    Helper to decode a base64url segment by reusing the utils function.
    """
    ...
    def decode_complete(
        ...
        **kwargs,
        ...
        if verify_signature and not algorithms:
            raise DecodeError(
                'You must pass a value for the "algorithms" argument when verifying a token.'
            )
        ...
        if header.get("b64", True) is False:
            if detached_payload is None:
                raise DecodeError(
                    'A "detached_payload" argument is required if the header parameter "b64" is false.'
                )
